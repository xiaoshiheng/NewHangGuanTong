...
#ifndef kLibraryCaches
#define kLibraryCaches [NSString stringWithFormat:@"%@/Library/Caches",NSHomeDirectory()]
#endif

#ifndef kCachesDocument
#define kCachesDocument [NSString stringWithFormat:@"%@/CachesDocument",kLibraryCaches]
#endif

#ifndef kCrashLogPath
#define kCrashLogPath [NSString stringWithFormat:@"/%@/kCrashLog",kCachesDocument]
#endif


@implementation NSData (Nomal)
...
+ (NSData *)dataFromBase64String:(NSString *)aString encoding:(NSStringEncoding)encode{
    ...
    NSData * result=[data base64Decoded];
    return result;
}

- (NSString *)base64EncodedStringEncoding:(NSStringEncoding)encoding{
    
    NSData * data=[self base64Encoded];
...
- (NSData *)dataUsingAES128EncryptWithkey:(NSData *)keyData withIV:(void *)iv_{
    ...
    return [[self class]cipherData:self withkey:keyData withIV:iv_ Operation:kCCEncrypt Algorithm:kCCAlgorithmAES keySize:kCCKeySizeAES128 options:kCCOptionPKCS7Padding];
...
- (NSData *)gzipData{
    return [NSData gzipData:self];
}

- (NSData *)ungzipData{
    return [NSData ungzipData:self];
}
@end
