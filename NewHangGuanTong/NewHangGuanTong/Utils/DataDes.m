...
@end
@implementation DataDes

+ (NSString *) encode:(NSString *)str key:(NSString *)key{
    if (str.length>0) {
        // doCipher 不能编汉字，所以要进行 url encode
        NSMutableString* str1 = [DataDes urlEncode:str];
        NSMutableString* encode = [NSMutableString stringWithString:[DataDes doCipher:str1 key:key context:kCCEncrypt]];
        //    [CXDDes formatSpecialCharacters:encode];
        return encode;
    }
    return @"";
...
+ (NSString *) decode:(NSString *)str key:(NSString *)key{
    ...
    [DataDes reformatSpecialCharacters:str1];
    NSString *rt = [DataDes doCipher:str1 key:key context:kCCDecrypt];
    return rt;
}
...
+ (NSString *)encodeBase64WithString:(NSString *)strData {
    return [DataDes encodeBase64WithData:[strData dataUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (NSString *)doCipher:(NSString *)sTextIn key:(NSString *)sKey
    ...
    if (encryptOrDecrypt == kCCDecrypt) {
        dTextIn = [[DataDes decodeBase64WithString:sTextIn] mutableCopy];
    }
    else{
    ...
    if (encryptOrDecrypt == kCCDecrypt){
        ...
        free(bufferPtr1);
        sResult = [DataDes encodeBase64WithData:dResult];
    }
    return sResult;
}
